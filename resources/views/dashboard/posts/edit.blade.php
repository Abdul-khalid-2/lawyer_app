                        ...
                        <form id="blogPostForm">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" id="excerpt" name="excerpt" rows="2" 
                                          placeholder="Brief description of your blog post">{{ old('excerpt', $post->excerpt) }}</textarea>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="hidden" id="tags" name="tags" value="{{ $post->tags ? implode(',', json_decode($post->tags, true)) : '' }}">
                            </div>
                        </form>
                    </div>
                </div>
                    ...
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                ...
                                <p class="text-muted mb-0">Review your changes and save when ready</p>
                            </div>
                            <div class="d-flex gap-2">
                                <a href="{{ route('blog-posts.index') }}" class="btn btn-secondary">
                                    <i class="fas fa-arrow-left me-2"></i>Back to Posts
                                </a>
                                <button type="button" class="btn btn-success" onclick="builder.updateBlogPost()">
    ...
    <script>
        ...
        class TagsManager {
            ...
            loadExistingTags() {
                ...
                if (tagsInput && tagsInput.value.trim() !== '') {
                    const existingTags = tagsInput.value.split(',').map(tag => tag.trim()).filter(tag => tag !== '');
        ...
        class BlogPostBuilder {
            ...
            loadExistingStructure() {
                @if($post->structure && isset($post->structure['elements']))
                    const existingElements = @json($post->structure['elements']);
                    if (existingElements && existingElements.length > 0) {
                        existingElements.forEach(element => {
                            this.addElementFromData(element);
                        });
            ...
            getImageUrl(path) {
                ...
                if (path.startsWith('lawyers/') || path.startsWith('blog-images/')) {
                    return '/website/' + path;
                }
                return path;
            }
